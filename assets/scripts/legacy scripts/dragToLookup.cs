// dragToLookup.cs
// 2025-10-29 by XERONAME

using UnityEngine;



public class dragToLookup : MonoBehaviour
{
    ...
    private GameObject obj_pottery; // game-object of pottery
    private Rigidbody rb_pottery; // rigid-body of pottery-object
    private string name_obj_pottery = "DummyPottery"; // name-string of pottery game-object
    ...
    void Start()
    {
        obj_pottery = GameObject.Find(name_obj_pottery); // find and assign the target game-object to variable

        // DEBUG MESSAGE
        if (obj_pottery == null) { Debug.LogWarning("GameObject of pottery not found!"); } // write the  warn message, if target object not found
        else {
            rb_pottery = obj_pottery.GetComponent<Rigidbody>();
    ...
    void Update()
    {
        ...
        if (STAT_MOUSE_LEFTKEY) {
            ...
            Debug.Log("velocity: " +drag_angleVelocity);

            // apply angular-velocity to rigidbody of pottery
            rb_pottery.angularVelocity = new Vector3(0.0f, drag_angleVelocity, 0.0f);
            //rb_pottery.AddTorque(Vector3.up *_calcVel *drag_rotationScale, ForceMode.Impulse);
        }
